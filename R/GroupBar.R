...
#' @param ylimit Adjust the position of the distinctive mark
#' @param loc Modify the position of the letter mark
#' @examples
#' # data(data_wt)
...
GroupBar = function(data = data_wt,num = c(4:6),result = result,sig_show ="abc",loc = 1.5,ylimit = 1.2 ){
  ...
  p<-ggplot(A, aes(x=group , y=mean,group = name ))+
    ...
    geom_text(data=A, aes(x=group, y=mean +SD1 ,label=groups),position = position_dodge(.9))+
