...
#
# # #--------案例#--------
# result = EdgerSuper(ps = ps,group  = "Group")
# head(result)
...
EdgerSuper = function(otu = NULL,tax = NULL,map = NULL,tree = NULL ,ps = NULL,group  = "Group",pvalue = 0.05,lfc =0,artGroup = NULL){
  # 导入R包#-------
  ...
  for (i in 1:dim(aaa)[2]) {
    ...
    # FDR检验，控制假阳性率小于5%
    de_lrt = decideTestsDGE(lrt, adjust.method="fdr", p.value=pvalue,lfc=lfc)#lfc=0这个是默认值
    summary(de_lrt)
    # 导出计算结果
    x=lrt$table
    x$sig=de_lrt
    ...
    x = data.frame(row.names = row.names(x),logFC = x$logFC,level = x$level)
    head(x)
